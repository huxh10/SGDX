...
class ParticipantController(object):
    ...
    def xstart(self):
        ...
        #ps_thread_arp.join()
        ps_thread_xrs.join()
        self.logger.info("Return from ps_thread.join()")
    ...
    def process_sdn_reach(self, parts_set):
        ...
        if self.cfg.isSupersetsMode():
            ...
            #    if prefix in self.prefix_2_VNH:
            #        garp_required_vnhs.append(self.prefix_2_VNH[prefix])
            #    else:
            #        self.logger.warn("prefix:%s not in prefix_2_VNH" % prefix)
