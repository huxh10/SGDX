...
BOOL ABYParty::InitCircuit(uint32_t bitlen, uint32_t maxgates) {
	...
	if (m_eRole == SERVER) {
		...
		//m_vSharings[S_YAO_PIPE] = new YaoServerPipeSharing(m_eRole, m_sSecLvl.symbits, m_pCircuit, m_cCrypt);
	}
	else {
		m_vSharings[S_YAO] = new YaoClientSharing(m_eRole, m_sSecLvl.symbits, m_pCircuit, m_cCrypt);
		//m_vSharings[S_YAO_PIPE] = new YaoClientPipeSharing(m_eRole, m_sSecLvl.symbits, m_pCircuit, m_cCrypt);
	}
	switch (bitlen) {
	case 8:
	...
	m_vSharings[S_BOOL_NO_MT] = new BoolNoMTSharing(m_eRole, 1, m_pCircuit, m_cCrypt);
...
BOOL ABYParty::ABYPartyListen() {
	...
	for(uint32_t i = 0; i < 2; i++) {
		...
		for(uint32_t j = 0; j < m_vSockets.size(); j++) {
			...
			//tempsocks[i][j] = new CSocket();//TODO: this was commented recently since it is unneccesary
		}
	}
