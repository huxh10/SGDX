...
uint32_t BooleanCircuit::PutANDGate(uint32_t inleft, uint32_t inright) {
	...
	if(m_eContext != S_BOOL_NO_MT) {
		...
		if (m_pGates[gateid].nvals != INT_MAX) {
			...
			/*if(m_eContext == S_YAO_PIPE) {
				//set
				m_pGates[gateid].depth = max(m_vNonLinOnLayer.min_depth, m_pGates[gateid].depth);
				//resize
				if(m_vNonLinOnLayer.max_depth < m_pGates[gateid].depth) {
					cout << "resizing" << endl;
					ResizeNonLinOnLayer(m_pGates[gateid].depth);
				}
				//increase
				m_vNonLinOnLayer.num_on_layer[m_pGates[gateid].depth]+=m_pGates[gateid].nvals;
				cout << "increased (" << m_pGates[gateid].depth << ") to: " << m_vNonLinOnLayer.num_on_layer[m_pGates[gateid].depth] << endl;
				//update min depth
				if(m_vNonLinOnLayer.num_on_layer[m_pGates[gateid].depth] > GARBLED_TABLE_WINDOW) {
					for(uint32_t i = m_vNonLinOnLayer.min_depth; i < m_vNonLinOnLayer.max_depth; i++) {
						if(m_vNonLinOnLayer.num_on_layer[i] < GARBLED_TABLE_WINDOW) {
							m_vNonLinOnLayer.min_depth = i;
							break;
						}
					}
				}
...
share* BooleanCircuit::PutYaoSharedSIMDINGate(uint32_t nvals, yao_fields* keys, uint32_t bitlen) {
	share* shr = new boolshare(bitlen, this);
	for(uint32_t i = 0; i < bitlen; i++) {
		shr->set_wire(i, PutYaoSharedSIMDINGate(nvals, keys[i]));
	}
	return shr;
}
...
vector<uint32_t> BooleanCircuit::PutTruthTableMultiOutputGate(vector<uint32_t> in, uint32_t out_bits,
	...
	assert(out_bits <= 8);//TODO: only up to 8 out bits are supported. In case more are needed, update boolsharing_no_mt.h:
...
share* BooleanCircuit::PutTruthTableGate(share* in, uint64_t* ttable) {
	...
	out->set_wire(0, PutTruthTableGate(in->get_wires(), 1, ttable));
	return out;
}
...
vector<uint32_t> BooleanCircuit::PutDepthOptimizedAddGate(vector<uint32_t> a, vector<uint32_t> b, BOOL bCARRY, bool vector_and) {
	...
	zero_share->set_wire(0, zerogate);
	zero_share->set_wire(1, zerogate);
 ...
 * TODO: the method is uneccessarily complex since it had to be done quickly. Go over the method and unify notation / group similar operations.
 */
vector<uint32_t> BooleanCircuit::PutLUTAddGate(vector<uint32_t> a, vector<uint32_t> b, BOOL bCARRY) {
	//TODO: not working atm if the values have different sizes. the smaller value would need to be padded with leading zeros
	...
	//step 3: build the inverse carry tree TODO: Recursive
...
vector<uint32_t> BooleanCircuit::PutMulGate(vector<uint32_t> a, vector<uint32_t> b, uint32_t resbitlen, bool depth_optimized, bool vector_ands) {
	...
	if(m_eContext== S_BOOL && vector_ands) {
		...
		for(uint32_t i = 0; i < rep; i++) {
			...
			zero_share->set_wire(i, zerogate);
		}

		for(uint32_t i = 0; i < rep; i++) {
			PutMultiMUXGate(&ina, &zero_share, inb->get_wire_as_share(i),  1, &(mulout[i]));
		}
		...
		for (uint32_t i = 0, ctr; i < rep; i++) {
			...
			for (uint32_t j = 0; j < rep && ctr < lim; j++, ctr++) {
				vAdds[i][ctr] = mulout[j]->get_wire(i);//(a[j], b[i]);
			}
	...
	while (survivors.size() > 1) {
		...
		for (unsigned i = 0; i < survivors.size();) {
			if (i + 1 >= survivors.size()) {
				...
				survivors[j++] = PutSizeOptimizedAddGate(survivors[i], survivors[i + 1], false); //TODO true/false?
				i += 2;
			}
		}
		survivors.resize(j);
...
uint32_t BooleanCircuit::PutLUTGEGate(vector<uint32_t> a, vector<uint32_t> b) {
...
share** BooleanCircuit::PutCondSwapGate(share* a, share* b, share* s, BOOL vectorized) {
	...
	vector<vector<uint32_t> > out = PutCondSwapGate(a->get_wires(), b->get_wires(), s->get_wire(0), vectorized);
	...
	while (m_vELMs.size() > 1) {
		...
		for (unsigned i = 0; i < m_vELMs.size();) {
			if (i + 1 >= m_vELMs.size()) {
				...
				//	cmp = bc->PutGTTree(m_vELMs[i], m_vELMs[i+1]);
				if (m_eContext == S_BOOL) {
					cmp = PutDepthOptimizedGEGate(m_vELMs[i], m_vELMs[i + 1]); //TODO use SizeOptimized for BGP
#ifdef USE_MULTI_MUX_GATES
					//Multimux
					cond->set_wire(0, cmp);
	...
	while (m_vELMs.size() > 1) {
		...
		for (unsigned i = 0; i < m_vELMs.size();) {
			if (i + 1 >= m_vELMs.size()) {
				...
				if (m_eContext == S_BOOL) {
					cmp = PutDepthOptimizedGEGate(m_vELMs[i+1], m_vELMs[i]); //TODO use SizeOptimizedGEGate for BGP
