...
uint32_t Circuit::PutSubsetGate(uint32_t input, uint32_t* posids, uint32_t nvals, bool copy_posids) {
	uint32_t gateid = m_cCircuit->PutSubsetGate(input, posids, nvals, copy_posids);
	UpdateLocalQueue(gateid);
	return gateid;
}
...
share* Circuit::PutSubsetGate(share* input, uint32_t* posids, uint32_t nvals, bool copy_posids) {
	...
	for(uint32_t i = 0; i < input->bitlength(); i++) {
		...
		tmp[i] = m_cCircuit->PutSubsetGate(input->get_wire(i), posids, nvals, copy_posids);
		nsubsetgates++;
	}
