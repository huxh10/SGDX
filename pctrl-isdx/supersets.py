...
class SuperSets(object):
    ...
    def get_vmac(self, pctrl, vnh):
        ...
        if route is None:
            self.logger.warn("prefix "+str(prefix)+" not found in local")
            bgp_instance.rib['local'].dump(self.logger)
            return vmac_addr

        next_hop = route.neighbor
        if next_hop not in nexthop_2_part:
            self.logger.warn("Next Hop "+str(next_hop)+" not found in get_vmac call!")
            return vmac_addr
...
def get_all_participants_advertising(pctrl, prefix):
    ...
    for route in routes:
        ...
        if next_hop in nexthop_2_part:
            ...
            pctrl.logger.warn("In subcall of prefix2part: Next hop "+str(next_hop)+" NOT in nexthop_2_part")
